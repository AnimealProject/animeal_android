...
fun TabsHost() {
    ...
    val navigator = LocalNavigator.current ?: error("CompositionLocal is null")
    ...
    var guestMessageVisible by rememberSaveable { mutableStateOf(false) }
    val showGuestMessage = { guestMessageVisible = true }

    val onNavigate: (TabsRoute) -> Unit = { route ->
        when {
            GuestSession.isGuest.value && !route.allowGuest -> showGuestMessage()
            else -> navigationController.navigate(route.name) {
    ...
    Scaffold(
        ...
        CompositionLocalProvider(
            ...
            NavigationTabs(navigationController, showGuestMessage)
        }
    }
...
private fun NavigationTabs(
    ...
    ScreenNavHost(
        ...
        ) { HomeScreen(onDisablingRouteForGuest) }
        screen(TabsRoute.Analytics.name) { AnalyticsScreen() }
        screen(TabsRoute.More.name) { MoreHost(onDisablingRouteForGuest) }
